...
class Serializer {
    ...
    static serializeOldGuild(client, originalGuildId, banCollection, guildData) {
        ...
        if (!guildToCopy.available) throw new Error('Guilde originale non disponible. Veuillez réessayer plus tard.');

        // General
        Logger.logMessage(`${guildData.step++}. Sérialisation des données générales`);
        ...
        if (debug) Logger.logMessage(`${guildData.step - 1}.1 Sérialisation ${guildData.roles.length} role(s)`);

        // Categories
        Logger.logMessage(`${guildData.step++}. Serializing category data`);
        guildData.categories = this.serializeCategories(guildToCopy);
        if (debug) Logger.logMessage(`${guildData.step - 1}.1 Sérialisation ${guildData.categories.length} category(ies)`);
        ...
        if (debug) Logger.logMessage(`${guildData.step - 1}.1 Sérialisation ${guildData.textChannel.length} text channel(s)`);
        ...
        if (debug) Logger.logMessage(`${guildData.step - 1}.1 Sérialisation ${guildData.voiceChannel.length} voice channel(s)`);

        // Emojis
        Logger.logMessage(`${guildData.step++}. Serializing emojis`);
        guildData.emojis = this.serializeEmojis(guildToCopy);
        if (debug) Logger.logMessage(`${guildData.step - 1}.1 Sérialisation ${guildData.emojis.length} emoji(s)`);

        // Bans
        Logger.logMessage(`${guildData.step++}. Serializing bans`);
        guildData.bans = this.serializeBans(banCollection);
        if (debug) Logger.logMessage(`${guildData.step - 1}.1 Sérialisation ${guildData.bans.length} ban(s)`);

        // Save data to file
        Logger.logMessage(`${guildData.step++}. Saving guild data to file`);
        fs.writeFileSync(`guildData${originalGuildId}.json`, JSON.stringify(guildData));
        Logger.logMessage(`${guildData.step++}. Sérialisation terminée et données sauvegardées`);
